...
$projectDir = "" #Путь к папке с проектом, все остальные папки по-умолчанию хранятся внутри неё.
$image_dir = $projectDir + "\images" # Путь к папке с изображениями. Внутри должны находится папки вида N_ConceptName
$output_dir = $projectDir + "\lora" # Директория сохранения LoRA чекпоинтов
...
$clip_skip = 1 # Использовать вывод текстового энкодера с конца N-ного слоя. Для NAI-подобных моделей указывать 2.
...
$text_encoder_lr = $unet_lr/2 # Скорость обучения текстового энкодера
...
$lr_warmup_ratio = 0 # Отношение количества шагов разогрева планировщика к количеству шагов обучения (от 0 до 1). Не имеет силы при планировщике constant
...
$scale_weight_norms=1.0 #Регуляризация по максимальной норме - это метод стабилизации обучения сети путем ограничения нормы весов сети. Она может быть эффективна для подавления чрезмерной подгонки LoRA и улучшения стабильности при использовании с другими LoRA. Рекамендуемое значение=1.
$network_dropout=0.3 #Выключение нейронов с указанной вероятностью. Помогает с переобучением. Значение от 0 до 1, кохя рекомендует от 0.1 до 0.3.

# Генерация примеров во время обучения
$sample_every_n_steps = 10 #Каждые n кол-во шагов генерировать пример
$sample_prompts = $image_dir + "\sample.txt" #Поместите файл sample.txt в папку с обучающими данными. В нём содержащий подсказки для вывода образца. В файле содержатся не только промпты, но и другие аргументы, см. документацию японца
...
$logging_dir = $projectDir + "\logs" # (опционально) Папка для логов
...
if ($is_structure_wrong -eq 0 -and $abort_script -ne "y")
{
	...
	$run_parameters += " --sample_every_n_steps=$sample_every_n_steps"
   	...
   	$run_parameters += " --sample_sampler=$sample_sampler"
    	...
    	#Нормализация и выброс нейронов
        $run_parameters += " --scale_weight_norms=$scale_weight_norms"
    	$run_parameters += " --network_dropout=$network_dropout"
